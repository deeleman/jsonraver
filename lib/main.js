...
var processRequest = function (requestObject, onComplete) {
    ...
    requestCallback = function (err, response, body) {
        if (!response) {
            buildError([requestObject.uri, 'returned NO RESPONSE']);
        } else if (!err && response.statusCode <= 200) {
            try {
                result = JSON.parse(body);
                if (result.errors) {
                    buildError([requestObject.uri, 'returned the following error:', result.errors.toString()], response.statusCode);
                }
            } catch (exception) {
                buildError([requestObject.uri, 'returned an illegal JSON message:', exception], response.statusCode);
            }
        } else if (response.statusCode > 200) {
            buildError([requestObject.uri, 'returned a', response.statusCode, 'HTTP status message'], response.statusCode);
        }

        if (err) {
            buildError([requestObject.uri, 'returned an error message.', err]);
        }
